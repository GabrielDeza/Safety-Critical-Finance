...
parser.add_argument('--max_iter', default=3, type=int, help='number of iterations on the adv dataset algorithm')
...
for iter in range(0,max_iter): #number of times to iterate over testing set
    ...
    adv_list_dataset = ListDataset([{FieldName.TARGET: target,
                                 FieldName.START: start,
                                 FieldName.FEAT_DYNAMIC_REAL: FDR}
                                for (target, start, FDR) in zip(testing_target_values, start_date, adversarial_fdr)], freq="H")
    adv_rolling_set = generate_rolling_dataset(
                                dataset=adv_list_dataset,
                                start_time=pd.Timestamp(df_target.index[train_length], freq="H"),
                                end_time=pd.Timestamp(df_target.index[test_length + prediction_length], freq="H"),
                                strategy=StepStrategy(prediction_length=prediction_length, step_size=step_size))
    for i, adv_dict in enumerate(adv_rolling_set): #iterating over the rolling dataset
        ...
        interim_adv_set = ListDataset([{
            FieldName.TARGET: target, FieldName.START: start, FieldName.FEAT_DYNAMIC_REAL: FDR}
                                for (target, start, FDR) in zip(target_adv, start_adv, fdr_adv)], freq ="H")
        mx.random.seed(seed)
        np.random.seed(seed)
        for test_entry, forecast in zip(interim_adv_set, predictor.predict(interim_adv_set)):
            for t in range(0,prediction_length):
                ...
                for tries in range(0,len(columns)):
                    ...
                    #print(adversarial_fdr[0].shape,grad.shape)
                    #print(idx, time_idx)
...
for i,(train_dict,adv_train_dict) in enumerate(zip(dataset_rolled,dataset_rolled_adv)):
    ...
    train_data_adv = ListDataset([{FieldName.TARGET: target, FieldName.START: start, FieldName.FEAT_DYNAMIC_REAL: FDR} for (target, start, FDR)
                                in zip(interim_target_adv, adv_start_, adv_fdr)], freq="H")
    mx.random.seed(seed)
    np.random.seed(seed)
...
adv_testing_target_vals = adv_testing_target_vals[5:]
adv_testing_pred_vals = adv_testing_pred_vals[5:]
adv_testing_dates = adv_testing_dates[5:]
...
#columns = ["Retweets", "Replies","Likes", "Volume","positive_score", "negative_score", "neutral_score","positive_percentage","negative_percentage","neutral_percentage","positive_count","negative_count", "neutral_count","General_score"]
columns = ["Retweets", "Replies","Likes", "Volume","positive_score", "negative_score", "neutral_score","positive_percentage","negative_percentage","neutral_percentage","positive_count","negative_count", "neutral_count","General_score"]
regular_fdr = pd.DataFrame(plotting_test_feat_dynamic[0].reshape((-1,len(columns))), columns = columns)
adversarial_fdr_plot = pd.DataFrame(adversarial_fdr[0].reshape((-1,len(columns))), columns = columns)
fig, axes = plt.subplots(nrows=len(columns), ncols=1, figsize = (10,10))

for i in range(0,len(columns)):
    regular_fdr[columns[i]].plot(ax=axes[i],sharex =axes[0],alpha = 0.75)
    adversarial_fdr_plot[columns[i]].plot(ax=axes[i],sharex =axes[0],alpha = 0.75)
    axes[i].set_title(f"{columns[i]}", loc='left', fontsize = 9)
    if i != 0:
...
new_dates = [pd.Timestamp(adv_testing_dates[i].astype(dtype='datetime64[D]')).strftime('%Y-%m-%d %H:%M:%S') for i in range(0,adv_testing_dates.shape[0])]
...
####################################################################
train_open_zero = df_open[pd.Timestamp(training_dates[0]).strftime('%Y-%m-%d %H:%M:%S')]
train_close_end = df_close[pd.Timestamp(training_dates[-1]).strftime('%Y-%m-%d %H:%M:%S')]
test_open_zero = df_open[pd.Timestamp(testing_dates[0]).strftime('%Y-%m-%d %H:%M:%S')]
test_close_end = df_close[pd.Timestamp(testing_dates[-1]).strftime('%Y-%m-%d %H:%M:%S')]
...
adv_greedy_gain_test, adv_test_invest, adv_testing_greedy_returns,adv_testing_greedy_PDT = greedy_gain(adv_testing_dates, adv_testing_pred_diff, adv_testing_target_diff,df_open,df_close,freq_type='hour')


#greedy gain_Kelly
kelly_greedy_gain_train,kelly_train_invest, kelly_training_greedy_returns,kelly_training_greedy_PDT,f_train = kelly_greedy_gain(training_dates, training_pred_diff,training_target_diff, df_open,df_close,freq_type='hour')
kelly_greedy_gain_test, kelly_test_invest, kelly_testing_greedy_returns,kelly_testing_greedy_PDT,f_test= kelly_greedy_gain(testing_dates, testing_pred_diff, testing_target_diff,df_open,df_close,freq_type='hour')
kelly_adv_greedy_gain_test, kelly_adv_test_invest, kelly_adv_testing_greedy_returns,kelly_adv_testing_greedy_PDT,f_adv = kelly_greedy_gain(adv_testing_dates, adv_testing_pred_diff, adv_testing_target_diff,df_open,df_close,freq_type='hour')
...
adv_threshold_gain_test,adv_testing_threshold_returns,adv_testing_threshold_PDT = threshold_gain(adv_testing_dates, adv_testing_pred_diff,adv_testing_target_diff,df_open,df_close,freq_type='hour')

#threshold gain kelly
kelly_threshold_gain_train, kelly_training_threshold_returns,kelly_training_threshold_PDT = kelly_threshold_gain(training_dates, training_pred_diff,training_target_diff,df_open,df_close,freq_type='hour')
kelly_threshold_gain_test,kelly_testing_threshold_returns,kelly_testing_threshold_PDT = kelly_threshold_gain(testing_dates, testing_pred_diff,testing_target_diff,df_open,df_close,freq_type='hour')
kelly_adv_threshold_gain_test,kelly_adv_testing_threshold_returns,kelly_adv_testing_threshold_PDT = kelly_threshold_gain(adv_testing_dates, adv_testing_pred_diff,adv_testing_target_diff,df_open,df_close,freq_type='hour')
...
new_dates = [pd.Timestamp(training_dates[i].astype(dtype='datetime64[D]')).strftime('%Y-%m-%d %H:%M:%S') for i in range(0,training_dates.shape[0])]
new_dates_plot = [pd.Timestamp(d).strftime('%Y-%m-%d %H:%M:%S') for d in training_dates]
