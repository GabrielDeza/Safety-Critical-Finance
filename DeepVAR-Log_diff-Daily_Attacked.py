...
parser.add_argument('--train_length', default=1150, type=int, help='Length of training set')
parser.add_argument('--validation_length', default=1190, type=int, help='Length of validation set')
parser.add_argument('--test_length', default=1290, type=int, help='Length of testing set')
parser.add_argument('--prediction_length', default=3, type=int, help='Prediction Length')
...
file_head = f"{company}_DeepVAR_Daily_"
...
filename = f"/Users/gabriel/Desktop/Fixed_Stock_Project/input_data/{company}_1d.csv"
...
for iter in range(0,max_iter): #number of times to iterate over testing set
    ...
    interim_adv_test_dataset = ListDataset(adv_time_series_dicts, freq="1B")
    adv_testing_dataset_rolled = generate_rolling_dataset(dataset=interim_adv_test_dataset,start_time=pd.Timestamp(df.index[validation_length], freq="1B"),
                                                      end_time=pd.Timestamp(df.index[test_length], freq="1B"),strategy=StepStrategy(prediction_length=prediction_length,step_size=prediction_length))
    ...
    for qq, train_dict in enumerate(adv_testing_dataset_rolled):
        interim_adv_testing_sets[qq % ((test_length - validation_length) // prediction_length)].append({"target": train_dict['target'], "start": pd.Timestamp(df.index[validation_length], freq="1B")})
...
adv_test_dataset = ListDataset(adv_time_series_dicts, freq="1B")
...
for i,train_dict in enumerate(adv_testing_dataset_rolled):
    adv_testing_sets[i%((test_length-validation_length)//prediction_length)].append({"target": train_dict['target'], "start": pd.Timestamp(df.index[validation_length], freq="1B")})
...
for j,(ts,adv_ts) in enumerate(zip(testing_sets,adv_testing_sets)): #iterating over the rolling sets
        ...
        interim_adv_dict = ListDataset(adv_ts, freq="1B")  # N times (1,truncated length)
...
for j,ts in enumerate(testing_sets): #iterating over the rolling sets
        ...
        for i, (target, forecast) in enumerate(zip(tss, forecasts)):
            testing_dates = np.concatenate((target.index[-prediction_length:], testing_dates))
            testing_target_vals = np.concatenate((target.values[-prediction_length:, 0].reshape((-1,)), testing_target_vals))
            testing_pred_vals = np.concatenate((np.mean(forecast.samples, axis=0)[:, 0].reshape((-1,)), testing_pred_vals))

correct_testing_target_vals = testing_target_vals[prediction_length:]
correct_testing_pred_vals = testing_pred_vals[prediction_length:]
correct_testing_dates = testing_dates[prediction_length:]
...
for j,ts in enumerate(testing_sets): #iterating over the rolling sets
        ...
        tss = list(ts_it)
        for i, (target, forecast) in enumerate(zip(tss, forecasts)):
            adv_testing_dates = np.concatenate((target.index[-prediction_length:], adv_testing_dates))
            adv_testing_target_vals = np.concatenate((target.values[-prediction_length:, 0].reshape((-1,)), adv_testing_target_vals))
            adv_testing_pred_vals = np.concatenate((np.mean(forecast.samples, axis=0)[:, 0].reshape((-1,)), adv_testing_pred_vals))

adv_correct_testing_target_vals = adv_testing_target_vals[prediction_length:]
adv_correct_testing_pred_vals = adv_testing_pred_vals[prediction_length:]
adv_correct_testing_dates = adv_testing_dates[prediction_length:]
...
if plottype == 'log_diff':
    ...
    new_dates = [pd.Timestamp(adv_correct_testing_dates[i].astype(dtype='datetime64[D]')).strftime('%Y-%m-%d') for i in range(0, adv_correct_testing_dates.shape[0])]
    print(f'--------- {len(new_dates)} {df_open[new_dates].to_numpy().shape} {adv_correct_testing_target_vals.shape[0]}')
...
train_open_zero = df_open[pd.Timestamp(correct_training_dates[0]).strftime('%Y-%m-%d')]
train_close_end = df_close[pd.Timestamp(correct_training_dates[-1]).strftime('%Y-%m-%d')]
test_open_zero = df_open[pd.Timestamp(correct_testing_dates[0]).strftime('%Y-%m-%d')]
test_close_end = df_close[pd.Timestamp(correct_testing_dates[-1]).strftime('%Y-%m-%d')]
...
adv_greedy_gain_test, adv_test_invest, adv_testing_greedy_returns,adv_testing_greedy_PDT = greedy_gain(adv_correct_testing_dates, adv_testing_pred_diff, adv_testing_target_diff,df_open,df_close)

#greedy gain_Kelly
kelly_greedy_gain_train,kelly_train_invest, kelly_training_greedy_returns,kelly_training_greedy_PDT,f_train = kelly_greedy_gain(correct_training_dates, training_pred_diff,training_target_diff, df_open,df_close)
kelly_greedy_gain_test, kelly_test_invest, kelly_testing_greedy_returns,kelly_testing_greedy_PDT,f_test= kelly_greedy_gain(correct_testing_dates, testing_pred_diff, testing_target_diff,df_open,df_close)
kelly_adv_greedy_gain_test, kelly_adv_test_invest, kelly_adv_testing_greedy_returns,kelly_adv_testing_greedy_PDT,f_adv = kelly_greedy_gain(adv_correct_testing_dates, adv_testing_pred_diff, adv_testing_target_diff,df_open,df_close)
...
adv_threshold_gain_test,adv_testing_threshold_returns,adv_testing_threshold_PDT = threshold_gain(adv_correct_testing_dates, adv_testing_pred_diff, adv_testing_target_diff,df_open,df_close)

#threshold gain kelly
kelly_threshold_gain_train, kelly_training_threshold_returns,kelly_training_threshold_PDT = kelly_threshold_gain(correct_training_dates, training_pred_diff,training_target_diff,df_open,df_close)
kelly_threshold_gain_test,kelly_testing_threshold_returns,kelly_testing_threshold_PDT = kelly_threshold_gain(correct_testing_dates, testing_pred_diff,testing_target_diff,df_open,df_close)
kelly_adv_threshold_gain_test,kelly_adv_testing_threshold_returns,kelly_adv_testing_threshold_PDT = kelly_threshold_gain(adv_correct_testing_dates, adv_testing_pred_diff,adv_testing_target_diff,df_open,df_close)
...
new_dates = [pd.Timestamp(correct_training_dates[i].astype(dtype='datetime64[D]')).strftime('%Y-%m-%d') for i in range(0,correct_training_dates.shape[0])]
new_dates_plot = [pd.Timestamp(d).strftime('%Y-%m-%d') for d in correct_training_dates]
