...
parser.add_argument('--test_length', default=500, type=int, help='Length of testing set')
parser.add_argument('--prediction_length', default=5, type=int, help='Prediction Length')
parser.add_argument('--epochs', default=20, type=int, help='number of epochs')
parser.add_argument('--batch_size', default=32, type=int, help='Batch size')
parser.add_argument('--nbpe', default=100, type=int, help='Number of batches per epoch')
...
parser.add_argument('--epsilon', default=0.1, type=float, help='Percent change in dataset at each iteration')
parser.add_argument('--frac_amount', default = 0.3, type= float, help ="% required for the while to stop")
parser.add_argument('--parameter', default='mu', type=str, help='parameter we want to change. its mu sigma nu for student-t',)
parser.add_argument('--adv_example_type', default='simple', type=str, help='parameter we want to change. its mu sigma nu for student-t',)
...
file_body = f"_prediction_length={prediction_length}_BS={batch_size}_NBpE={num_batches_per_epoch}_epochs={epochs}" \
               f"_parameter={parameter}_dir={direction}_eps={epsilon}_adv_example_type={adv_example_type}"
...
perturbations = {'Type of Algorithm': f"{adv_example_type}_{direction}_while_{parameter}_regular_grad", 'perturbation size': epsilon,
...
########################################################################
iter = 0
#get original distr_param over the entire rolling set:
original_distr_params = np.zeros(training_target_vals.shape[0]+20)
...
for i, adv_dict in enumerate(adv_rolling_set):  # iterating over the rolling dataset
    ...
    for test_entry, forecast in zip(interim_adv_set, predictor.predict(interim_adv_set)):
        for t in range(0, prediction_length):
            ...
            curr_param_value = gradients[parameter + '_actual']
            original_distr_params[cnter] = curr_param_value
            cnter +=1
...
next_distr_params =  original_distr_params.copy()
frac = 1 + direction*frac_amount
if direction ==1:
    while (np.mean(next_distr_params) < frac*np.mean(original_distr_params)) and iter <= 250:
            #number of times to iterate over testing set
            print(f"On iter {iter} original val = {np.mean(original_distr_params)} need to beat ={frac*np.mean(original_distr_params)} curr val = {np.mean(next_distr_params)} ")
            iter+=1
            ...
            for i, adv_dict in enumerate(adv_rolling_set): #iterating over the rolling dataset
                ...
                for test_entry, forecast in zip(interim_adv_set, predictor.predict(interim_adv_set)):
                    for t in range(0,prediction_length):
                        ...
                        grad = gradients[parameter][t, 4:]
                        curr_param_value = gradients[parameter+'_actual']
                        next_distr_params[cnter] = curr_param_value
                        cnter+=1
...
if direction == -1:
    while (np.mean(next_distr_params) > frac*np.mean(original_distr_params)) and iter <= 250:
        ...
        adv_list_dataset = ListDataset([{FieldName.TARGET: target,
                                        ...
                                        zip(testing_target_values, start_date, adversarial_fdr)], freq="1B")
        adv_rolling_set = generate_rolling_dataset(
            dataset=adv_list_dataset,
            start_time=pd.Timestamp(df_target.index[train_length], freq="1B"),
            end_time=pd.Timestamp(df_target.index[test_length + prediction_length], freq="1B"),
