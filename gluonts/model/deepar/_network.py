...
class DeepARPredictionNetwork(DeepARNetwork):
    @validated()
    def __init__(self, num_parallel_samples: int = 100,grad_str:str =None, **kwargs) -> None:
        super().__init__(**kwargs)
    ...
    def sampling_decoder(
        ...
        #print(f"Inside of DeepAR, FDR is of shape = {time_feat.shape}")
        #print(f"Inside of DeepAR, target is of shape = {past_target.shape}")
        ...
        ######################## END OF CODE SECTION ##################################
            ...
            for k in range(self.prediction_length):
               ...
               # print(f"for fun, {repeated_time_feat.slice_axis(axis=1, begin=k, end=k + 1)}")
                #print(f"wow mu has a shape of {distr.base_distribution.mu.shape} {np.mean(distr.base_distribution.mu.asnumpy(),axis=0)[0]} {np.std(distr.base_distribution.mu.asnumpy(),axis=0)[0]}")
                gradient_mu_feat = autograd.grad(distr.base_distribution.mu, [time_feat], create_graph=True)
                gradient_sigma_feat = autograd.grad(distr.base_distribution.sigma, [time_feat], create_graph=True)
                gradient_nu_feat = autograd.grad(distr.base_distribution.nu, [time_feat], create_graph=True)
                #print('wow', gradient_mu_feat[0].shape)
                #print(f"wow lets try = {gradient_mu_feat[0].asnumpy()[0,k,:].squeeze().shape}")
                grad_dict ={}
                grad_dict['mu_actual'] = np.mean(distr.base_distribution.mu.asnumpy(),axis=0)[0]
                grad_dict['mu'] = gradient_mu_feat[0].asnumpy()[0,:,:].squeeze()

                grad_dict['sigma_actual'] = np.mean(distr.base_distribution.sigma.asnumpy(), axis=0)[0]
                grad_dict['sigma'] = gradient_sigma_feat[0].asnumpy()[0,:,:].squeeze()

                grad_dict['nu_actual'] = np.mean(distr.base_distribution.nu.asnumpy(), axis=0)[0]
                grad_dict['nu'] = gradient_nu_feat[0].asnumpy()[0, :, :].squeeze()
                file_path = self.grad_str +f"_{k}.p"
                pickle.dump(grad_dict, open(file_path,'wb'))
                ######################## END OF CODE SECTION ##################################
