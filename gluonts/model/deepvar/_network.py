    ...
    def sampling_decoder(
        ...
        with autograd.record():
            ...
            for k in range(self.prediction_length):
                ...
                ########################## This is where the gradient computation will lie ###########################
                #gradients are encapsulated in a list for some reason
                gradient_mu_feat = autograd.grad(distr.base_distribution.mu[:,:,0], past_target_cdf, create_graph=True)
                gradient_sigma_feat = autograd.grad(distr.base_distribution.D[:,:,0], past_target_cdf, create_graph=True)
                grad_dict = {}
...
class DeepVARPredictionNetwork(DeepVARNetwork):
    @validated()
    def __init__(self, num_parallel_samples: int,grad_str:str = None, **kwargs) -> None:
        super().__init__(**kwargs)
